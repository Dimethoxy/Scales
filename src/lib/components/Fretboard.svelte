					...
					{#each Array(numFrets + 1) as _, fret}
						...
						<div class="relative flex h-12 min-w-12 flex-1 items-center justify-center">
							...
							{#if note && (!onlyShowActiveNotes || scaleDegree >= 0)}
								<button
									type="button"
									class="text-md relative z-10 flex h-11 w-11 cursor-pointer items-center justify-center rounded-full border border-ctp-surface0 font-bold text-ctp-crust transition-transform hover:scale-105 focus:outline-none active:scale-95 {scaleDegree >=
									0
